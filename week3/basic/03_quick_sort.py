...
def partition(arr, low, high):
    ...
    for curindex in range(low, high):
        ...
        if(arr[curindex] <= pivot):
            indexOfMin += 1
            arr[indexOfMin], arr[curindex] = arr[curindex], arr[indexOfMin]
            
    
    # TODO: 피벗을 올바른 위치(i+1)에 배치
    indexOfPivot = indexOfMin+1
    arr[indexOfPivot], arr[high] = arr[high], arr[indexOfPivot]
    
    return indexOfPivot #피봇이 있는 인덱스 반환
